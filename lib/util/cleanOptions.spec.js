...
describe('cleanOptions', function () {
    ...
    it('should handle a dir string with relative path', function () {
        expect(cleanOptions_1.cleanOptions({ dir: 'MY_RELATIVE_PATH' })).toEqual(__assign(__assign({}, DEFAULT_OPTIONS), { dir: path.resolve('MY_RELATIVE_PATH'), index: "file://" + path.join(path.resolve('MY_RELATIVE_PATH'), 'index.html') }));
    });
    it('should handle a dir string with absolute path', function () {
        expect(cleanOptions_1.cleanOptions({ dir: '/home/me/MY_ABSOLUTE_PATH' })).toEqual(__assign(__assign({}, DEFAULT_OPTIONS), { dir: '/home/me/MY_ABSOLUTE_PATH', index: 'file:///home/me/MY_ABSOLUTE_PATH/index.html' }));
    });
    it('should handle a false index', function () {
        expect(cleanOptions_1.cleanOptions({ index: false })).toEqual(__assign(__assign({}, DEFAULT_OPTIONS), { index: false }));
    });
